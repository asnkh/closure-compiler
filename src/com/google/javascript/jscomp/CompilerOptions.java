...
public class CompilerOptions implements Serializable, Cloneable {
    ...
    /**
     * A common enum for compiler passes that can run either globally or
     * locally.
     */
    public enum Reach {
        ALL, LOCAL_ONLY, NONE
    ...
    /**
     * Only do transpilation, don't inject es6_runtime.js or do any
     * optimizations (this is useful for per-file transpilation).
     */
    ...
    /**
     * Whether to infer consts. This should not be configurable by external
     * clients. This is a transitional flag for a new type of const analysis.
     * 
    ...
    /**
     * Whether the compiler should assume that a function's "this" value never
     * needs coercion (for example in non-strict "null" or "undefined" will be
     * coerced to the global "this" and primitives to objects).
     */
     ...
     * <li>The compiler will always try to process all inputs fully, even if it
     * encounters errors.</li>
     * <li>The compiler may record more information than is strictly needed for
     * codegen.</li>
     * </ul>
     */
    ...
    /**
     * If true, name anonymous functions only. All others passes will be
     * skipped.
     */
    boolean nameAnonymousFunctionsOnly;

    /**
     * Configures the compiler to run expensive sanity checks after every pass.
     * Only intended for internal development.
     */
    DevMode devMode;

    /**
     * Configures the compiler to log a hash code of the AST after every pass.
     * Only intended for internal development.
     */
    ...
    /**
     * Checks the integrity of references to qualified global names. (e.g.
     * "a.b")
     */
    ...
    /**
     * Regex of string literals that may only appear in goog.getCssName
     * arguments.
     */
    ...
    public void setCheckEventfulObjectDisposalPolicy(CheckEventfulObjectDisposal.DisposalCheckingPolicy policy) {
        ...
        // The CheckEventfulObjectDisposal pass requires types so enable
        // inferring types if
    ...
    /**
     * A comma separated white-list of global names. When
     * {@link #aliasExternals} is enable, if set to a non-empty string, only
    ...
    /**
     * A comma separated white-list of global names. When
     * {@link #aliasExternals} is enable, these global names will not be
     * aliased.
     */
     ...
     * contains certain words from being aliased. If the value is the empty
     * string, no words are blacklisted.
     */
    ...
    /**
     * Use a renaming heuristic with better stability across source changes.
     * With this option each symbol is more likely to receive the same name
     * between builds. The cost may be a slight increase in code size.
     */
    boolean preferStableNames;

    /**
     * Generate pseudo names for variables and properties for debugging
     * purposes.
     */
    ...
    /**
     * Use @nosideeffects annotations, function bodies and name graph to
     * determine if calls have side effects. Requires --check_types.
     */
    ...
    /**
     * Rename private properties to disambiguate between unrelated fields based
     * on the coding convention.
     */
    boolean disambiguatePrivateProperties;

    /**
     * Rename properties to disambiguate between unrelated fields based on type
     * information.
     */
    ...
    /**
     * Replace UI strings with chrome.i18n.getMessage calls. Used by Chrome
     * extensions/apps.
     */
    ...
    /**
     * A JS function to be used for logging run-time type assertion failures. It
     * will be passed the warning as a string and the faulty expression as
     * arguments.
     */
    ...
    /**
     * Normally, when there are an equal number of single and double quotes in a
     * string, the compiler will use double quotes. Set this to true to prefer
     * single quotes.
     */
     ...
     * run through the compiler. These scripts are then put into HTML. By
     * default, we assume strings are untrusted. If the compiler is run from the
     * command-line, we assume that strings are trusted.
     */
     ...
     * compilation. This is implicitly true if {@link #externExportsPath} is
     * set.
     */
    ...
    /**
     * Whether to return strings logged with AbstractCompiler#addToDebugLog in
     * the compiler's Result.
     */
    boolean useDebugLog;

    /**
     * Charset to use when generating code. If null, then output ASCII. This
     * needs to be a string because CompilerOptions is serializable.
     */
    ...
    /**
     * Data holder Alias Transformation information accumulated during a
     * compile.
     */
     ...
     * 
     * Command-line frontends to the compiler should set these properties like a
     * builder.
     */
    ...
    /**
     * Creates a map of String->Node from a map of
    ...
    /**
     * Sets the value of the {@code @define} variable in JS to a boolean
     * literal.
     */
    ...
    /**
     * Whether the warnings guard in this Options object enables the given group
     * of warnings.
     */
    ...
    /**
     * The emergency fail safe removes all strict and ERROR-escalating warnings
     * guards.
     */
    ...
    /**
     * Sets the variable and property renaming policies for the compiler, in a
     * way that clears warnings about the renaming policy being uninitialized
     * from flags.
     */
     ...
     * String$f = x). This should only be used with the typed version of the
     * externs files.
     */
    ...
    /**
     * A renaming map instance to use to signal the use of the "inconsistent" id
     * generator type.
     */
    public static final RenamingMap UNIQUE_ID_GENERATOR = ReplaceIdGenerators.UNIQUE;
    ...
    /**
     * Enable run-time type checking, which adds JS type assertions for
     * debugging.
     * 
     * @param logFunction
     ...
     * 
     * @param entryPoints
     *            Entry points to the program. Must be goog.provide'd symbols.
     *            Any goog.provide'd symbols that are not a transitive
     *            dependency of the entry points will be deleted. Files without
     ...
     * (print summary if type checking is on, see --check_types), 3 (always
    ...
    /**
     * Sets ECMAScript version to use for the input. If you are not transpiling
     * from one version to another, use #setLanguage instead.
     */
    ...
    /**
     * Sets ECMAScript version to use for the output. If you are not transpiling
     * from one version to another, use #setLanguage instead.
     */
    ...
    boolean needsConversion() {
        return languageOut != LanguageMode.NO_TRANSPILE && languageIn != languageOut;
    }

    /**
     * Whether to include "undefined" in the default types. For example:
     * "{Object}" is normally "Object|null" becomes "Object|null|undefined"
     * "{?string}" is normally "string|null" becomes "string|null|undefined" In
     * either case "!" annotated types excluded both null and undefined.
     */
     ...
     * 
     * This is mostly used for piping the warnings and errors to a file behind
     * the scenes.
     * 
     * If you want to filter warnings and errors, you should use a
     * WarningsGuard.
     * 
    ...
    /**
     * Whether to keep internal data structures around after we're finished
     * compiling. We do this by default when IDE mode is on.
     */
    ...
    public void setExtractPrototypeMemberDeclarations(boolean enabled) {
        this.extractPrototypeMemberDeclarations = enabled ? ExtractPrototypeMemberDeclarationsMode.USE_GLOBAL_TEMP
    ...
    /**
     * @param value
     *            Whether to enable private property disambiguation based on the
     *            coding convention.
     */
    ...
    /**
     * Rewrites ES6 modules so that modules can be concatenated together, by
     * renaming all globals to avoid conflicting with other modules.
     */
    ...
    public interface AliasTransformationHandler {
         ...
         * caller.
         * <p>
         * Callers are allowed to request multiple AliasTransformation instances
         * for the same file, though it is expected that the first and last char
         * values for multiple instances will not overlap.
         * <p>
         * This method is expected to have a side-effect of storing off the
         * created AliasTransformation, which guarantees that invokers of this
         * interface cannot leak AliasTransformation to this implementation that
         * the implementor did not create
         * 
         * @param sourceFile
         *            the source file the aliases re contained in.
         * @param position
         *            the region of the source file associated with the
         *            goog.scope call. The item of the SourcePosition is the
         *            returned AliasTransformation
         */
     ...
     * <p>
     * While aliases defined by goog.scope are expected to by only 1 per file,
     * and the only top-level structure in the file, this is not enforced.
     */
    public interface AliasTransformation {
         ...
         * multiple times (since this is generally the behavior in JavaScript
         * code).
         * 
         * @param alias
